...
/// <summary>
/// 头像框数据表行。
/// 定义每条头像框的配置数据：名称、图标路径、解锁方式等。
/// </summary>
public sealed class HeadPortraitFrameDataRow : DataRowBase, ICodeDataRow
{
    /// <summary>
    /// 列拆分分隔符。
    /// </summary>
    ...
    /// 合法头像框 Code 的前缀。
    /// </summary>
    private const string CodePrefix = "head_portrait_frame_";
    ...
    public override bool ParseDataRow(string dataRowString, object userData)
    {
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because row string is empty.");
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because column count '{0}' is invalid, row '{1}'.", columns.Length, dataRowString);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because Id '{0}' is invalid.", columns[0]);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because Code '{0}' is invalid.", columns[1]);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because Name is empty, code '{0}'.", code);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because IconPath is empty, code '{0}'.", code);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because IsDefaultUnlocked '{0}' is invalid, code '{1}'.", columns[4], code);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because AcquireType is empty, code '{0}'.", code);
            return false;
        }
        ...
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because AcquireParam '{0}' is invalid, code '{1}'.", columns[6], code);
            return false;
        }

        // 默认解锁的头像框，AcquireParam 必须为 0
        if (isDefaultUnlocked && acquireParam != 0)
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because default unlocked frame '{0}' must have AcquireParam = 0.", code);
            return false;
        }

        // 非默认解锁且获取类型为 gold 的头像框，AcquireParam 必须大于 0
        if (!isDefaultUnlocked && string.Equals(acquireType, "gold", StringComparison.OrdinalIgnoreCase) && acquireParam <= 0)
        {
            Log.Warning("HeadPortraitFrameDataRow parse failed because locked gold frame '{0}' must have AcquireParam > 0.", code);
            return false;
        }
