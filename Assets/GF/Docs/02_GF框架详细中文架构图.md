...
## 7. 资源路径与加载模型（当前配置：内核路由自动透明双后端）

GF 内核 `LoadAsset` 入口注入了 Addressables 路由，**业务方完全无需关心资源在哪个后端**——同一相对路径，资源在哪里就从哪里加载。

```mermaid
flowchart TD
    A["业务调用<br/>GameEntry.UI.OpenUIForm / Resource.LoadAsset / Entity.ShowEntity / DataTable.Load / Sound.PlaySound<br/>GameAssets.TryLoadAsset"]
    A --> B[ResourceComponent.LoadAsset]
    B --> C[GF 内核 ResourceManager.ResourceLoader.LoadAsset]
    C --> D{Addressables 路由判定<br/>同步遍历 ResourceLocators 调 Locate}

    D -- 命中 Addressables 群组 --> E1[Addressables.LoadAssetAsync&lt;UnityEngine.Object&gt;]
    E1 --> F1["登记反查表<br/>s_HandlesByAsset"]
    F1 --> G1[按 LoadAssetCallbacks 协议派发完成]

    D -- 未命中 / catalog 未就绪 --> E2[GF 主链路]
    E2 --> F2["ResourceMode.Resource = 4<br/>+ ResourcesLoadResourceAgentHelper"]
    F2 --> G2[Resources.LoadAsync]

    G1 --> R[资源就绪 → 业务回调]
    G2 --> R

    style D fill:#fff3cd,stroke:#856404,color:#000
    style E1 fill:#d4edda,stroke:#155724,color:#000
...
```mermaid
flowchart TD
    A1[GF 内核引用归零<br/>m_AssetPool.Unspawn] --> B1[HybridResourceHelper.Release]
    B1 --> C1{反查 s_HandlesByAsset}
    C1 -- 命中 --> D1[Addressables.Release&#40;handle&#41;]
    C1 -- 未命中 --> D2[base.DefaultResourceHelper.Release<br/>Resources 资源行为]
```

当前配置含义：

1. **路由优先 + Resources 兜底**：catalog 就绪后，资源若在 Addressables 群组就走 Addressables；否则走 `ResourceMode.Resource → Resources.LoadAsync`。catalog 未就绪/初始化失败时全部退回 GF 主链路。
2. **业务侧零改动**：UIComponent / EntityComponent / DataTableComponent / SoundComponent / GameAssetModule 全部经 `m_ResourceManager.LoadAsset` 收敛，路由在内核入口生效。
3. **路径字符串完全一致**：同一相对路径既可作为 Resources 路径，也可作为 Addressables Address，由实际资源所在位置决定加载链路。
4. **句柄安全**：`HybridResourceHelper.Release` 反查 `s_HandlesByAsset` 字典精确释放 Addressables 句柄，零泄漏。
5. **强制后端**：业务侧若需绕过路由，可直调 `GameEntry.Resource.LoadAddressableAssetAsync<T>` / `GameEntry.AddressableAssets.LoadAsset`（扩展方法），强制走 Addressables。
6. **冷启动**：`Addressables.InitializeAsync` 在 `GameEntry.Start` 阻塞等待完成才执行 `InitCustomComponents`，确保 GameAssets 首批预加载即可命中路由；包体模式约 50-200ms 延迟。
